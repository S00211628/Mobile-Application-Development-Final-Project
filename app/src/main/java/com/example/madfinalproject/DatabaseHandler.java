...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public List<HiScore> getTopFiveScores() {
        ...
        // Select All Query to get Length

        SQLiteDatabase db = this.getReadableDatabase();
        long count = DatabaseUtils.queryNumEntries(db, TABLE_HI_SCORES);

        Log.v("Table Size", String.valueOf(count));

        if (count > 5){
            // Select 5 Query
            selectQuery = "SELECT  * FROM " + TABLE_HI_SCORES +
                    " ORDER BY SCORE DESC " +
                    " LIMIT 5";
        }
        else{
